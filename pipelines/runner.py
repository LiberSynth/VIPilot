...
def _runner_msg(batch_id, pipeline_name, phase, err_type=None):
    msg = f"batch={{}}, pipeline={pipeline_name}, phase={phase}"
    if err_type:
        msg += f", type={err_type}"
...
def _handle_batch_error(e, batch_id, category):
    ...
    if isinstance(e, AppException):
        ...
        write_log_entry(batch_id, 'runner', _runner_msg(batch_id, pipeline_name, "error_handled", "AppException"), level='silent')
    else:
        ...
        write_log_entry(batch_id, 'runner', _runner_msg(batch_id, pipeline_name, "error_handled", type(e).__name__), level='silent')
...
def run_batch(batch_id, pipeline, category):
    ...
    write_log_entry(batch_id, 'runner', _runner_msg(batch_id, category, "run_start"), level='silent')
    try:
        pipeline.run(batch_id, category)
        write_log_entry(batch_id, 'runner', _runner_msg(batch_id, category, "run_done"), level='silent')
    except Exception as e:
        _handle_batch_error(e, batch_id, category)
    finally:
        environment.release_batch(batch_id)
        environment.wakeup_loop()
        write_log_entry(batch_id, 'runner', _runner_msg(batch_id, category, "released"), level='silent')


def start_batch_thread(batch_id, pipeline, pipeline_name):
    """Запускает поток для обработки батча."""
    write_log_entry(batch_id, 'runner', _runner_msg(batch_id, pipeline_name, "thread_start"), level='silent')
