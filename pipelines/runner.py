...
def _handle_batch_error(e, batch_id, pipeline_name, log_id):
    if isinstance(e, AppException):
        ...
        write_log_entry(log_id, f"[runner] batch={batch_id}, pipeline={pipeline_name}, phase=error_handled, type=AppException", level='silent')
    else:
        ...
        write_log_entry(log_id, f"[runner] batch={batch_id}, pipeline={pipeline_name}, phase=error_handled, type={type(e).__name__}", level='silent')
...
def run_batch(batch_id, pipeline, log_id):
    ...
    write_log_entry(log_id, f"[runner] batch={batch_id}, pipeline={pipeline_name}, phase=run_start", level='silent')
    try:
        pipeline.run(batch_id, log_id)
        write_log_entry(log_id, f"[runner] batch={batch_id}, pipeline={pipeline_name}, phase=run_done", level='silent')
    except Exception as e:
        _handle_batch_error(e, batch_id, pipeline_name, log_id)
    finally:
        environment.release_batch(batch_id)
        environment.wakeup_loop()
        write_log_entry(log_id, f"[runner] batch={batch_id}, pipeline={pipeline_name}, phase=released", level='silent')
...
def start_batch_thread(batch_id, pipeline, pipeline_name):
    ...
    write_log_entry(log_id, f"[runner] batch={batch_id}, pipeline={pipeline_name}, phase=thread_start", level='silent')
