...
def _vk_story(video_file, group_id, log_id):
...
def _vk_wall(video_file, group_id, log_id):
...
def _publish_vk(batch, log_id):
    ...
    if do_story:
        ...
        story_ok = _vk_story(video_file, group_id, log_id) is not None

    if do_wall:
        if log_id:
            db_log_entry(log_id, 'Публикую на стену…')
        wall_ok = _vk_wall(video_file, group_id, log_id) is not None
