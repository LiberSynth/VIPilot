...
*   **Entry point:** `main.py` (agents must not edit without explicit user instruction; tamper check in `scripts/check_main_py.sh`)
*   **Core Application Logic:** `main.py`, `common/`, `pipelines/`, `utils/`
...
- **Logging:** Inline `write_log_entry(batch_id, channel, message, level)` at the call site; `app_log` and other logging wrappers are forbidden.
...
*   **`main.py` is frozen:** Agents must not edit `main.py` on their own. New logic belongs in `common/`, `pipelines/`, or `routes/`. If a task touches `main.py`, or the file was changed, or `check_main_py.sh` fails — **stop and report to the developer**; do not refactor, squeeze, or update the baseline to bypass the check. Unexpected edits are caught at commit time by `scripts/check_main_py.sh` (baseline line count, not a documented norm).
*   `pre-commit` hook: runs `scripts/check_main_py.sh` (main.py tamper signal) and build bump via `set_build_number.py`. Mandatory; do not disable or bypass.
...
*   **Publish overlay (Playwright):** `docs/publish-overlay-convention.md`
