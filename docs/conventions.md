...
## Правило 9: `main.py` — точка входа, правки только по указанию пользователя

`main.py` — точка входа: bootstrap приложения и `main_loop`. Новую логику сюда не добавлять — выносить в `common/`, `pipelines/`, `routes/` и т.д.

**Самостоятельные изменения `main.py` агентами и автоматизацией строго запрещены.** Правка допустима только по **явному указанию пользователя**.

**Если задача требует правки `main.py`, или файл уже изменён, или `check_main_py.sh` / pre-commit сообщает о расхождении — агент обязан остановиться и сообщить разработчику.** Не править `main.py`, не выносить код «для прохождения проверки», не обновлять `MAIN_PY_LINE_COUNT` в `scripts/check_main_py.sh`, не обходить hook. Дальше — только после решения разработчика: откат, явное разрешение на правку, или обновление baseline им самим.

**Контроль при коммите:** `scripts/check_main_py.sh` (вызывается из pre-commit и `check_conventions.sh`) сравнивает число строк с зафиксированным baseline. Это не норма из конвенций, а сигнал: «main.py тронули». При расхождении коммит блокируется — разработчик просматривает diff, откатывает изменения или обновляет baseline в `scripts/check_main_py.sh`.
...
| Правка `main.py` | **Запрещена** без явного указания; при необходимости или сбое проверки — **сообщить разработчику**, не обходить |
| Новая логика сервиса | `common/`, `pipelines/`, `routes/` — не в `main.py` |
