/* VK Video browser widget — авторизация через встроенный браузер */

(function () {
  var VIEWPORT_W = 900;
  var VIEWPORT_H = 680;

  var canvas  = document.getElementById('vkvideo-browser-canvas');
  var wrap    = document.getElementById('vkvideo-browser-wrap');
  var overlay = document.getElementById('vkvideo-browser-overlay');
  var btnSave = document.getElementById('vkvideo-btn-save');
  ...
  function getTargetId() {
    var el = document.getElementById('vkvideo_target_id');
    return el ? el.value : '';
  }

  function getStudioUrl() {
    var card = document.getElementById('vkvideo-browser-card');
    return card ? (card.dataset.studioUrl || '') : '';
  }

  function sendEvent(ev) {
    if (!active) return;
    fetch('/api/vkvideo-browser/event', {
      method: 'POST',
  ...
  function connectStream() {
    ...
    firstFrame = false;
    sse = new EventSource('/api/vkvideo-browser/stream');
  ...
  window.vkvideoBrowserOpen = function () {
    if (active) return;
    var tid = getTargetId();
    if (!tid) return;
    ...
    fetch('/api/vkvideo-browser/start', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  window.vkvideoBrowserSaveSession = function () {
    var tid = getTargetId();
    if (!tid) return;
    if (btnSave) btnSave.disabled = true;

    fetch('/api/vkvideo-browser/save-session', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ target_id: tid }),
    })
  ...
  setInterval(function () {
    ...
    var tid = getTargetId();
    if (!tid) return;
    var url = '/api/vkvideo-browser/status?target_id=' + encodeURIComponent(tid);
    fetch(url)
      .then(function (r) { return r.json(); })
      .then(function (data) {
  ...
  window.addEventListener('beforeunload', function () {
    if (active) {
      fetch('/api/vkvideo-browser/stop', { method: 'POST', keepalive: true }).catch(function () {});
    }
  });
