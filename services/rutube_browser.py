...
def _process_event(page, ev: dict):
    ...
    try:
        ...
        write_log_entry(None, f"[rutube_browser] Ошибка события {ev_type!r}: {e}", level='silent')
...
def _browser_loop(target_id: str):
    ...
    os.makedirs(RUTUBE_PROFILE_DIR, exist_ok=True)
    write_log_entry(None, f"[rutube_browser] Профиль Chrome: {RUTUBE_PROFILE_DIR}", level='silent')

    try:
        with sync_playwright() as pw:
            context = pw.chromium.launch_persistent_context(
                user_data_dir=RUTUBE_PROFILE_DIR,
                headless=True,
                args=[
                    "--no-sandbox",
            ...
            try:
                ...
                write_log_entry(None, f"[rutube_browser] Ошибка навигации: {e}", level='silent')
            ...
            while _running:
                if _save_request_event.is_set():
                    ...
                    try:
                        ...
                        cookies = context.cookies(["https://rutube.ru"])
                ...
                try:
                    ...
                    write_log_entry(None, f"[rutube_browser] Ошибка скриншота: {e}", level='silent')
        ...
        write_log_entry(None, f"[rutube_browser] Критическая ошибка: {e}", level='silent')
        return
        ...
        with sync_playwright() as pw:
            ...
            if cookies:
                try:
                    ...
                    write_log_entry(None, f"[rutube_pipeline] Загружено {len(cookies)} куков", level='silent')
                except Exception as e:
                    write_log_entry(None, f"[rutube_pipeline] Ошибка куков: {e}", level='silent')
