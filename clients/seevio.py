"""
Seevio API client (https://seevio.ai, base URL https://api.seevio.ai).
Отвечает за HTTP-взаимодействие с платформой Seevio: submit-запрос, poll-цикл.
Модели Seedance 2.0 доступны через API Seevio, а не напрямую у Seedance.
...
_SEEVIO_KEY = os.environ.get('SEEVIO_API_KEY', '')
...
def _headers():
    return {
        'Authorization': f'Bearer {_SEEVIO_KEY}',
...
def build_body(body_tpl, prompt, ar_x, ar_y, video_duration, batch_id=None, category=None):
    """Строит тело запроса Seevio из шаблона модели и параметров пайплайна."""
...
def submit(batch_id, category, model_name: str, submit_url: str, platform_url: str,
           ...
           video_duration: int):
    """
    Отправляет задание на генерацию видео в Seevio (одна попытка).
...
def poll(batch_id, category, status_url: str, response_url: str):
    """
    Поллит Seevio до завершения генерации.
    ...
    for attempt in range(_POLL_MAX):
        ...
        try:
            ...
            try:
                ...
                write_log_entry(
                    batch_id, category,
                    f"Seevio poll [{attempt + 1}] не-JSON (HTTP {resp.status_code})",
                    level='warn',
                )
                write_log_entry(
                    batch_id, category,
                    f"Seevio poll [{attempt + 1}] http={resp.status_code}, body={_compact_json(resp.text)}",
                    level='silent',
                )
            ...
            write_log_entry(
                batch_id, category,
                f"Seevio poll [{attempt + 1}] http={resp.status_code}, status={status}, keys={list(data.keys()) if isinstance(data, dict) else []}",
                level='silent',
            )
            ...
            if resp.status_code >= 400:
                ...
                if 'moderation' in err_lc or 'policy' in err_lc:
                    msg = f'Seevio: генерация отклонена модерацией контента: {err_text}'
                    write_log_entry(batch_id, category, msg, level='error')
                    return None, msg
                if _is_fatal(resp.status_code):
                    msg = f'Seevio: фатальная ошибка провайдера (HTTP {resp.status_code}): {err_text}'
                    write_log_entry(batch_id, category, msg, level='error')
                    return None, msg
                write_log_entry(batch_id, category, f"Seevio poll HTTP {resp.status_code}: {err_text}", level='warn')
                continue
            ...
            if status == 'completed':
                ...
                if not video_url:
                    msg = f"Seevio: нет video URL в ответе: {str(data)}"
                    write_log_entry(batch_id, category, msg, level='error')
                    return None, msg
            ...
            if status == 'failed':
                ...
                msg = f'Seevio: генерация провалилась: {reason}'
                write_log_entry(batch_id, category, msg, level='error')
                return None, msg
            ...
            write_log_entry(
                batch_id, category,
                f"Seevio poll [{attempt + 1}] exception={type(e).__name__}: {e}",
                level='silent',
            )

    msg = 'Таймаут генерации видео Seevio (2 часа)'
    write_log_entry(batch_id, category, msg, level='error')
    return None, msg
