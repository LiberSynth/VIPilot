...
def publish(
    ...
    try:
        ...
        _state = {"clip_url": ""}
        _stop_snap = threading.Event()

        def _snap_loop(page):
            """Фоновый поток: скриншот раз в секунду пока не выставлен _stop_snap."""
            while not _stop_snap.wait(timeout=1.0):
                try:
                    from services.browser_registry import get_browser as _gb
                    _b = _gb("vkvideo")
        ...
        def _do_publish(page, _ctx):
            snap_thread = threading.Thread(target=_snap_loop, args=(page,), daemon=True)
            snap_thread.start()
            try:
