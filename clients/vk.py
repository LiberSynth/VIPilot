...
- Стена + ВКВидео (клип): video.save(is_short_video=1, wallpost=1) → upload
...
def publish_wall(video_data: bytes, group_id: int, log_id) -> int | None:
    ...
    Использует video.save с is_short_video=1 — публичный API, доступен без спецразрешений.
    """
    pub_title = build_publication_title()
    filename  = publication_file_name(pub_title)
    ...
    if log_id:
        write_log_entry(log_id, fmt_id_msg('ВК: начинаю загрузку клипа video_id={}', video_id))
    ...
    for attempt in range(3):
        ...
        if log_id:
            write_log_entry(log_id, 'Все попытки загрузки клипа провалились', level='error')
        return None

    if log_id:
        write_log_entry(log_id, fmt_id_msg('Клип загружен и опубликован в ВКВидео: video_id={}, owner_id={}',
                                           video_id, owner_id))
    return video_id
