...
@bp.route("/text-models/<model_id>/note", methods=["POST"])
def api_text_model_note(model_id):
    if not is_authenticated():
        return jsonify({"error": "unauthorized"}), 401
    data = request.get_json(silent=True) or {}
...
@bp.route("/text-models/<model_id>/body", methods=["POST"])
def api_text_model_body(model_id):
    if not is_authenticated():
        return jsonify({"error": "unauthorized"}), 401
    data = request.get_json(silent=True) or {}
    body = data.get("body")
...
@bp.route("/targets/<target_id>/active", methods=["POST"])
def api_target_active(target_id):
    if not is_authenticated():
        return jsonify({"error": "unauthorized"}), 401
    data = request.get_json(silent=True) or {}
    if "active" not in data or not isinstance(data["active"], bool):
        return jsonify({"error": "active must be a boolean"}), 400
    ok = db_set_target_active(target_id, data["active"])
    return jsonify({"ok": ok})
...
def api_vacuum_db():
    ...
    try:
        ...
        write_log_entry(None, "system", f"[api] Дефрагментация БД: ошибка — {e}", level='error')
        return jsonify({"ok": False, "error": str(e)}), 500
...
def api_workflow_start():
    ...
    write_log_entry(None, "system", "[api] Движок запущен вручную", level='silent')
...
def api_workflow_pause():
    ...
    write_log_entry(None, "system", "[api] Движок приостановлен вручную", level='silent')
...
def api_workflow_deep_debugging():
    ...
    write_log_entry(None, "system", f"[api] Глубокая отладка {label}", level='info')
...
def api_delete_batch(batch_id):
    ...
    try:
        ...
        write_log_entry(None, "system", f"[api] Ошибка удаления батча: {e}", level="silent")
...
def api_workflow_restart():
    if not is_authenticated():
        return jsonify({"error": "unauthorized"}), 401
    write_log_entry(None, "system", "[api] Перезапуск приложения вручную", level='info')
