...
Повторяющийся код вынесен в `pipelines/base.py`: `check_cancelled(pipeline_name, batch_id, batch)` и `handle_critical_error(pipeline_name, batch_id, log_id, e)` — все четыре пайплайна используют эти функции вместо копипасты.
...
- `routes/admin.py` — Blueprint: веб-роуты (/, /admin, /producer, /save, /save-dzen, /logout, /favicon, /healthz)
...
- `pipelines/base.py` — общие утилиты пайплайнов: `check_cancelled`, `handle_critical_error`
- `pipelines/planning.py` — MSK импортируется из utils.consts
- `pipelines/story.py` — проверяет возвращаемое значение `db_set_batch_story`; при False — логирует ошибку и завершается без `batch_done=True`
...
- `templates/admin.html` — HTML-скелет панели администратора (5 вкладок); использует common/header.html и макрос info_panel
...
- `templates/common/header.html` — общий заголовок, подключается через {% include %} в admin.html и producer.html
