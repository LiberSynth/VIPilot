...
- `text_prompt` — текстовый промпт для генерации сюжетов (Текстовый промпт)
- `format_prompt` — форматный промпт: инструкции модели (Форматный промпт)
...
- `templates/common/controls.html` — общие UI-контролы: макрос modes_card(approve_stories, use_donor) — карточка «Режимы» (Утверждать сюжеты + Подбирать видео из пула); используется в root.html (Рабочий поток) и production.html (Режиссер)
