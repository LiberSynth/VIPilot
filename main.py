...
def generate_video():
    ...
    if is_emulation():
        try:
            log_msg('[ЭМУЛЯЦИЯ] Пропускаю генерацию, беру случайное ранее сгенерированное видео...')
            url = db_get_random_video_url()
            if not url:
                log_msg('[ЭМУЛЯЦИЯ] В базе нет сохранённых видео — сначала запустите хотя бы один цикл без эмуляции', 'error')
                return False
            log_msg(f'[ЭМУЛЯЦИЯ] Видео выбрано, скачиваю...')
